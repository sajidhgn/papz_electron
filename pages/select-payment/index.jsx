...
const SelectPaymentPage = () => {
    ...
    //     try {
    //         // Fetch available printers and ports
    //         const printerList = await window.printerElectronAPI.getPrinters();
    //         const ports = await window.printerElectronAPI.getSerialPorts();

    //         setPrinters(printerList);
    //         setAvailablePorts(ports);

    //         // Set default printer and port (or use the first one if available)
    //         const initialPrinter = printerList.find(
    //             (printer) => printer.name === GlobalConfig.printerID
    //         ) || printerList[0];

    //         const initialPort = ports.find(
    //             (port) => port.path === GlobalConfig.portName
    //         ) || ports[0];

    //         if (initialPrinter) setSelectedPrinter(initialPrinter.name);
    //         if (initialPort) setSelectedPort(initialPort.path);

    //         // Initialize the printer and serial port
    //         if (initialPort) await handleSerialPortInit(initialPort.path);
    //         if (initialPrinter) await handlePrintTest(initialPrinter.name);
    ...
    // const handlePrintTest = async (printerName) => {
    //     try {
    //         const result = await window.printerElectronAPI.printTest(printerName);
    //         setStatus(result);
    //     } catch (error) {
    ...
    // const handleSerialPortInit = async (portName) => {
    //     try {
    //         const result = await window.printerElectronAPI.initializeSerialPort(portName, 9600);
    //         setStatus(result);
    //     } catch (error) {
    //         console.error('Error initializing serial port:', error);
    //         setStatus('Serial port initialization failed');
    //     }
    // };

    const handleCheckConnection = () => {
        window.machineElectronAPI.checkMachineConnection();
      };
      ...
      useEffect(() => {
        ...
        window.machineElectronAPI.ipcRenderer.on('machine-connection-response', handleResponse);
    
        return () => {
          window.machineElectronAPI.ipcRenderer.removeListener('machine-connection-response', handleResponse);
        };
      }, []);
